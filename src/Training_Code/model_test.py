...
# 코랩 환경의 경로로 설정
MODEL_PATH = '/content/drive/MyDrive/KFall_dataset/models/tflite_model/fall_detection_compatible.tflite'
SCALER_MEAN_PATH = '/content/drive/MyDrive/KFall_dataset/models/tflite_model/extracted_scaler_mean.npy'
SCALER_SCALE_PATH = '/content/drive/MyDrive/KFall_dataset/models/tflite_model/extracted_scaler_scale.npy'
TEST_DATA_PATH = '/content/drive/MyDrive/KFall_dataset/models/training_data/Test'  # 테스트 데이터 경로

SEQ_LENGTH = 50  # 시퀀스 길이
N_FEATURES = 9    # 특징 수 (AccX, AccY, AccZ, GyrX, GyrY, GyrZ, EulerX, EulerY, EulerZ)

# 디버그 모드 활성화 (문제 해결을 위한 상세 로그 출력)
DEBUG_MODE = True
...
class FallDetectorTest:
    def __init__(self, model_path, scaler_mean_path, scaler_scale_path, seq_length=50, n_features=9):
        ...
        if DEBUG_MODE:
            ...
            print(f"스케일러 평균 샘플: {self.scaler_mean[:3]}")
            print(f"스케일러 스케일 샘플: {self.scaler_scale[:3]}")
    ...
    def normalize_data(self, data):
        ...
        try:
            ...
            # 디버그 모드에서 입력 데이터 샘플 출력
            if DEBUG_MODE:
                print(f"정규화 전 데이터 샘플: {data[0, :3]}")
            ...
            # 디버그 모드에서 정규화 후 데이터 샘플 출력
            if DEBUG_MODE:
                print(f"정규화 후 데이터 샘플: {data_norm[0, :3]}")
    ...
    def predict(self, data_sequence):
        ...
        try:
            ...
            # 디버그 모드에서 입력 데이터 형태 출력
            if DEBUG_MODE:
                print(f"predict 입력 데이터 형태: {data_sequence.shape}")
            ...
            # 디버그 모드에서 출력 데이터 형태 및 값 출력
            if DEBUG_MODE:
                print(f"출력 데이터 형태: {output_data.shape}, 값: {output_data}")
            ...
            # 디버그 모드에서 최종 결과 출력
            if DEBUG_MODE:
                print(f"예측 결과: {prediction}, 확률: {fall_prob}")
    ...
    def evaluate_on_test_data(self, X_test, y_test):
        """테스트 데이터셋에 대한 평가 수행"""
        predictions = []  # 예측 결과 저장 리스트
        probabilities = []  # 확률 저장 리스트
        ...
        try:
            for i in range(X_test.shape[0]):
                ...
                # 디버그 모드에서 샘플 데이터 형태 출력
                if DEBUG_MODE and (i < 5 or i % 500 == 0):
                    print(f"샘플 {i} 데이터 형태: {sample_data.shape}")
                ...
                # 진행 상황에 따라 중간 결과 보고
                if i % 500 == 0 and i > 0:
                    print(f"현재까지 예측 결과 수: {len(predictions)}")
        ...
        # 예측 결과 배열을 파일로 저장 (디버깅용)
        if DEBUG_MODE:
            np.save('/content/predictions.npy', predictions)
            np.save('/content/probabilities.npy', probabilities)
            print("예측 결과가 파일로 저장되었습니다.")

        # 성능 평가
        print("\n=== 모델 성능 평가 결과 ===")

        # 길이 확인
        min_len = min(len(predictions), len(y_test))  # 항상 min_len 정의
    ...
    def simulate_realtime_detection(self, test_sequence, true_label, show_plot=True):
        ...
        try:
            ...
            for i in range(test_sequence.shape[0]):
                ...
                if len(self.data_buffer) >= self.seq_length:
                    ...
                    if DEBUG_MODE and i % 10 == 0:
                        print(f"프레임 {i}, 데이터 형태: {data_array.shape}")
            ...
            # 결과 저장
            if len(predictions) > 0:
                print(f"시뮬레이션 결과: {len(predictions)}개 예측 완료")
                print(f"평균 확률: {np.mean(predictions):.2f}, 최대 확률: {np.max(predictions):.2f}")
            else:
...
def load_test_data():
    ...
    try:
        ...
        # 데이터 샘플 출력 (디버깅용)
        if DEBUG_MODE:
            print(f"X_test 첫 번째 샘플 형태: {X_test[0].shape}")
            print(f"X_test 첫 번째 샘플 일부: {X_test[0, 0, :3]}")
...
def main():
    ...
    try:
        ...
        if DEBUG_MODE:
            ...
            sample_data = X_test_resized[sample_idx]
            print(f"테스트 샘플 형태: {sample_data.shape}")
            result = detector.predict(sample_data)
            print(f"테스트 샘플 결과: {result}")
            if result is None:
                print("단일 샘플 테스트 실패! 더 이상 진행할 수 없습니다.")
                return

        # 테스트 데이터 일부만 사용 (디버깅 모드에서)
        if DEBUG_MODE:
            test_size = min(100, X_test_resized.shape[0])
            print(f"디버깅 모드: 처음 {test_size}개 샘플만 테스트")
            X_test_subset = X_test_resized[:test_size]
            y_test_subset = y_test[:test_size]

            # 전체 테스트 데이터셋에 대한 평가
            predictions, probabilities = detector.evaluate_on_test_data(X_test_subset, y_test_subset)
        else:
            # 전체 테스트 데이터셋에 대한 평가
        ...
        # 길이 확인
        if len(predictions) != len(y_test_subset if DEBUG_MODE else y_test):
            y_test_summary = y_test_subset if DEBUG_MODE else y_test
            min_len = min(len(predictions), len(y_test_summary))
            predictions_summary = predictions[:min_len]
            y_test_summary = y_test_summary[:min_len]
